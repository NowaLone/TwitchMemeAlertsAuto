...
namespace TwitchMemeAlertsAuto.Core.Services
{
	public class MemeAlertsService : IMemeAlertsService
	{
		...
		public async Task<bool> SendMemeAsync(Sticker sticker, string senderName, bool isSoundOnly = false, bool fullscreen = false, CancellationToken cancellationToken = default)
		{
			var streamerId = await GetStreamerIdAsync(cancellationToken).ConfigureAwait(false);

			using var request = new HttpRequestMessage(HttpMethod.Post, "api/sticker/send") { Content = new StringContent($"{{\"toChannel\":\"{streamerId}\",\"stickerId\":\"{sticker.Id}\",\"isSoundOnly\":{isSoundOnly.ToString().ToLower()},\"topic\":\"Last\",\"name\":\"{senderName}\",\"isMemePartyActive\":false,\"message\":\"\",\"deviceType\":\"desktop\",\"fullscreen\":{fullscreen.ToString().ToLower()} }}", new MediaTypeHeaderValue(MediaTypeNames.Application.Json)) };
			using var responseMessage = await DoRequest(request, cancellationToken).ConfigureAwait(false);
